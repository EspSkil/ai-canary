...
function applyNorwegianCopy(root=document.body){
  ...
  applyIndicatorCardLanguage(root===document.body ? document.getElementById("indicatorDashboardGrid") : root.querySelector?.("#indicatorDashboardGrid"));
}
...
function applyIndicatorCardLanguage(root=document.getElementById('indicatorDashboardGrid')){
  ...
  const replacements=[
    ...
    ['Current compute pricing does not show a clear oversupply warning; guaranteed capacity still carries a premium.','Dagens compute-prising gir ikke et tydelig varsel om overtilbud; garantert kapasitet handles fortsatt med en premie.'],
    ['The current v1 model uses SOX momentum as a fast market checkpoint; fundamental semiconductor inputs are not yet scored.','Dagens v1-modell bruker SOX-momentum som et raskt markedssignal; fundamentale halvlederdata scores foreløpig ikke.'],
    ['CAPEX risk is broad, with the highest company-level pressure concentrated in the most aggressive infrastructure builders.','CAPEX-risikoen er bred, med høyest selskapsrisiko hos de mest aggressive infrastrukturbyggerne.'],
  ...
  nodes.forEach(node=>{
    ...
    replacements.forEach(([en,no])=>{ if(text.includes(en)) text=text.split(en).join(no); });
    node.nodeValue=text;
  });
}
